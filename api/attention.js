...
async function jfetch(url,opts={},timeout=5000){
...
async function duneQuery(sym,key,force,errors){
  ...
  try{
    const ex=await jfetch("https://api.dune.com/api/v1/sql/execute",{method:"POST",headers,body:JSON.stringify({sql,performance:"small"})},5000);
    ...
    for(let i=0;i<4;i++){
      ...
      const st=await jfetch(`https://api.dune.com/api/v1/execution/${id}/status`,{headers},5000);
      status=st.state;
    }
    ...
    const res=await jfetch(`https://api.dune.com/api/v1/execution/${id}/results?limit=1`,{headers},5000);
